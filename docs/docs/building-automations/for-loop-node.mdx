...
|---------------------|--------------------------------------------------------------------------------------------------------------------------|
| `nodes`             | List of `action_node` or `if_else_node` nodes.                                                                           |
| `reset_nodes`       | List of `action_node` or `if_else_node` nodes.                                                                           |
| `on_error_in_loop`  | Action to execute if an error occurs in the loop. Can be `continue`, `break`, or `raise`. Default is to raise an error.  |
...
Inside a `for_loop_node`, `index` is a special variable that represents the current iteration:

If `order_ids = ["ORD-001", "ORD-002", "ORD-003"]`:

- First iteration: `index = 0` → `"{order_ids[index]}"` evaluates to `"ORD-001"`
- Second iteration: `index = 1` → `"{order_ids[index]}"` evaluates to `"ORD-002"`
- Third iteration: `index = 2` → `"{order_ids[index]}"` evaluates to `"ORD-003"`
...
- `for_loop_node` requires the variable to be populated before the loop executes
